...
def skratena_path(path1):
    path_short = []
    point_previous = path1[0]
    sur1 = 0
    sur2 = 0
    index = 0
    index2 = 0

    for point_curent in path1:
        if index == 0:
            path_short.append(point_curent)
            index2 += 1
        elif index == 1:
            pass
        elif (point_curent[0] - point_previous[0]) == sur1 and (point_curent[1] - point_previous[1]) == sur2:
            pass
        elif (point_curent[0] - point_previous[0]) != sur1 or (point_curent[1] - point_previous[1]) != sur2:
...
def init():
    ...
    path2 = skratena_path(path1)

    grid_with_path1 = draw_path(path2, copy.deepcopy(filtered_data))

    grid_with_path1_converted = convert_to_numeric(grid_with_path1)

    #prem_krat_path = skratena_path(path1)
